...
SoulTaker의 캐릭터는 `Character`를 중심으로 구성됩니다.
...
### DamageSystem
데미지 계산은 `DamageSystem`을 통해 처리합니다.
현재 코드에는 피격 연출이 임시로 일부 포함되어 있지만, 구조적으로 핵심은 "히트 목록을 순회하며 실제 대미지를 적용하는 공통 진입점"이라는 점입니다.

`DamageSystem`은 다음 역할을 담당합니다.
1. 피격 대상이 실제로 공격 가능한 오브젝트인지 확인
2. 공격력 기반으로 최종 데미지 계산
3. `LifeFeature`를 통해 실제 체력 감소 적용
4. 필요 시 스턴, 무적 등 상태 처리 연계

```c#
private static int ComputeDamage(FieldObject source, FieldObject target, int baseDamage)
{
    int srcAtk = 0;
    if (source.TryGetFeature<StatsFeature>(out var sStats))
        srcAtk = sStats.CurrentStats.Attack;

    return Math.Max(0, baseDamage + srcAtk);
}
```

즉, 공격 판정은 `AttackEmitterFeature`가 만들고, 실제 피격 반영은 `DamageSystem`이 처리하며, 체력 변화는 `LifeFeature`가 담당하는 식으로 책임을 나눴습니다.

## Enemy AI

이제 적 AI는 `EnemyController`를 사용하지 않고 `EnemyBTRunner`와 `EnemyBTNodes`를 중심으로 구성합니다.
...
현재는 `Test`, `ObjectPool` 같은 컨테이너를 관리하며, `UniTask.WhenAll`로 여러 테이블을 동시에 읽도록 구성했습니다.
...
UI는 `UIManager`, `UINavigator`, `UISceneRoot`, `ImmortalCanvas`를 중심으로 구성했습니다.

### UIWindow / UIPopup 분리
UI는 크게 Window와 Popup으로 나뉩니다.
Window는 화면의 메인이 되는 UI이고, Popup은 그 위에 중첩 가능한 보조 UI입니다.
이 구조는 `UINavigator`에서 스택으로 관리합니다.
...
뒤로 가기 같은 입력도 이 구조를 기준으로 처리합니다.
즉, 팝업이 있으면 팝업부터 닫고, 없으면 이전 윈도우로 돌아가게 됩니다.
...
이 덕분에 어떤 씬에서도 Canvas가 없어서 UI가 뜨지 않는 문제를 줄일 수 있습니다.

### UISceneRoot와 ImmortalCanvas
`UISceneRoot`는 Window Canvas, Popup Canvas, 각 Root Transform, 배경 Blur 같은 UI 루트 참조를 보관합니다.
그리고 `ImmortalCanvas`는 씬 전환과 무관하게 유지되어야 하는 공통 UI를 담당합니다.
